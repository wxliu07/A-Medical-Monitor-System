...
if __name__ == "__main__":
    ...
    if not video:
        ...
        cap = cv2.VideoCapture("../video/A01.avi")
    ...
    win = pg.PlotWidget(title="plotting")
    ...
    p1 = win.plot(title="FFT")
    p2 = win.plot(title="Signal")
    ...
    def update():
        ...
        p1.setData(x=freqs_of_interest, y=fft_of_interest, pen='g')
        # p1.setYRange(-1.2, 1.2)

        p2.clear()
        # p2.plot(filtered_data[20:], pen='g')
        p2.setData(y=filtered_data[20:], pen='g')

        # 获取p2的视图框并设置Y轴范围
        p2_viewbox = win.getViewBox()
        p2_viewbox.setYRange(-1.2, 1.2)

        # p2.setYRange(-1.2, 1.2)
